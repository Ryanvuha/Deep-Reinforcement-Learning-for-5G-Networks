...
for M in X:
    try:
        df_i = pd.read_csv('figures_M={}/convergence.txt'.format(M), sep=',', header=None, index_col=False).dropna()
        df_i['M'] = M
        Y = pd.concat([Y, df_i])
    except:
        pass
...
df_convergence = Y.groupby('M').mean().reset_index()
#df_convergence = Y.groupby('M').quantile(0.8).reset_index()
df_convergence.columns = ['M', 'episode']
...
plot_secondary(X,Y1,Y3,Y2,Y4, r'$M$', r'Maximum Transmit Power [dBm]', r'Maximum Achievable SINR [dB]', 47, 90, 'achievable_sinr_power.pdf')
