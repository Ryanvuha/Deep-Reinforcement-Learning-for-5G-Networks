...
def compute_distributions(optimal=False):
    ...
    for M in [4, 8, 16, 32, 64]:
        if optimal:
            ...
            df_1 = pd.read_csv('figures_M={}/ue_1_sinr.txt'.format(M), sep=',', header=None, index_col=False).transpose().dropna()
            df_2 = pd.read_csv('figures_M={}/ue_2_sinr.txt'.format(M), sep=',', header=None, index_col=False).transpose().dropna()
        
        cutoff = 5 + 10*np.log2(M)
        ...
        if optimal:
            ...
            df_3 = pd.read_csv('figures_M={}/ue_1_power.txt'.format(M), sep=',', header=None, index_col=False).transpose().dropna()
            df_4 = pd.read_csv('figures_M={}/ue_2_power.txt'.format(M), sep=',', header=None, index_col=False).transpose().dropna()
        
        df = pd.concat([df_3, df_4], axis=0, ignore_index=True)
        df = df.astype(float)
        
        tx_power = np.array(df)
        df_power = pd.DataFrame(tx_power)
...
#    ax.xaxis.set_major_locator(MultipleLocator(1))
 ...
 #   ax.xaxis.set_major_formatter(FuncFormatter(lambda x, pos: int(2**x)))
    ...
    ax.set_ylim(40, y1max)
    ax_sec.set_ylim(20, y2max)
    
    plt.grid(True)
    plt.legend([plot1_, plot2_, plot3_, plot4_], ['TX Power JB-PCIC', 'TX Power Optimal', 'SINR JB-PCIC', 'SINR Optimal'], loc='best')
    fig.tight_layout()
    plt.savefig('{}'.format(filename), format='pdf')
    matplotlib2tikz.save('{}.tikz'.format(filename))
...
plot_ccdf(df_final_opt_[['sinr_4', 'sinr_8', 'sinr_16', 'sinr_32', 'sinr_64']], [r'$M = 4$', r'$M  = 8$', r'$M = 16$', r'$M = 32$', r'$M = 64$'])
...
for M in X:
    try:
        df_i = pd.read_csv('figures_M={}/rewards.txt'.format(M), sep=',', header=0, index_col=False).dropna()
        df_i['M'] = M
        Y = pd.concat([Y, df_i])
    except:
        pass
    
Y['Reward'] = Y['Reward'].astype(int)

Y.to_csv('convergence_vs_ant.csv', index=False)
# TDOO: Group by M, but find the episode with the minimum reward above r_max
df_convergence = Y.groupby('M').min().reset_index() # agg({'Reward': 'min'})

#df_convergence = Y.groupby('M').quantile(0.95).reset_index()
plot_primary(df_convergence['M'], df_convergence['Episode'], r'$M$', r'Convergence Episode ($\zeta$)', MIN_EPISODES_DEEP, MAX_EPISODES_DEEP, 'convergence.pdf')
...
plot_secondary(X,Y1,Y3,Y2,Y4, r'$M$', r'Maximum Transmit Power [dBm]', r'Maximum Achievable SINR [dB]', 47, 67, 'achievable_sinr_power.pdf')
